...
object Js {
    const val CACHE_VAR = "_browserForTvPreviouslyFocusedElement"
    ...
    object MP4TranslationWorkaround {
        private const val CACHED_SCROLL_POSITION = "_browserTV_cachedScrollPosition"
...
var _browserTV_lastTimeout;
var _browserTV_isScrollPositionObserverLoaded;
(function () {
    /* Ensure we only add one listener. */
    if (_browserTV_isScrollPositionObserverLoaded) { return; }
    _browserTV_isScrollPositionObserverLoaded = true;

    console.log('Browser TV workaround - adding scroll event listener');
    window.addEventListener('scroll', (e) => {
        ...
        if (typeof _browserTV_lastTimeout !== 'undefined') {
            ...
            window.clearTimeout(_browserTV_lastTimeout);
        }

        var pendingScrollPosition = window.scrollY;
        _browserTV_lastTimeout = window.setTimeout(() => {
            $CACHED_SCROLL_POSITION = pendingScrollPosition;
            console.log('Browser TV workaround - caching scroll position: ' + $CACHED_SCROLL_POSITION);
        }, 500);
    });
})();""".trimIndent()
...
(function () {
    ...
    window.setTimeout(() => {
        console.log('Browser TV workaround - scrolled fullscreen to non-fullscreen scroll position: ' + nonFullscreenPageScrollPosition);
...
var _browserTV_playbackStateObserverJava;
var _browserTV_isPlaybackStateObserverLoaded;
(function () {
    ...
    const javaInterface = _browserTV_playbackStateObserverJava;
    if (!javaInterface) {
    ...
    if (!_browserTV_isPlaybackStateObserverLoaded) {
        _browserTV_isPlaybackStateObserverLoaded = true;
...
""".trimIndent()
    ...
    (function () {
        if (typeof _browserTV_inputAddedObserver !== 'undefined') return;
        ...
        _browserTV_inputAddedObserver = new MutationObserver(mutationList => {
        ...
        _browserTV_inputAddedObserver.observe(document, {subtree: true, childList: true});
